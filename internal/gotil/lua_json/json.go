...
func Encode(value lua.LValue) (data []byte, err error) {
	bf := bytes.NewBuffer([]byte{})
	jsonEncoder := json.NewEncoder(bf)
	jsonEncoder.SetEscapeHTML(false)
	if err = jsonEncoder.Encode(jsonValue{LValue: value, visited: make(map[*lua.LTable]bool)}); err != nil {
		return
	}
	data = bf.Bytes()
	return
...
func (j jsonValue) MarshalJSON() (data []byte, err error) {
	...
	case lua.LBool:
		if err = jsonEncoder.Encode(bool(converted)); err == nil {
			data = bf.Bytes()
		}
	case lua.LNumber:
		if err = jsonEncoder.Encode(float64(converted)); err == nil {
			data = bf.Bytes()
		}
	case *lua.LNilType:
		data = []byte(`null`)
	case lua.LString:
		if err = jsonEncoder.Encode(string(converted)); err == nil {
			data = bf.Bytes()
		}
	case *lua.LTable:
		...
		case lua.LTNumber:
			...
			if err = jsonEncoder.Encode(arr); err == nil {
				data = bf.Bytes()
			}
		case lua.LTString:
